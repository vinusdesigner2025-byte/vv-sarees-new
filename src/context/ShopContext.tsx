...
type ShopContextValue = {
  wholesaleWishlist: ShopProduct[];
  retailWishlist: ShopProduct[];
  ...
  wholesaleWishlistCount: number;
  retailWishlistCount: number;
  wholesaleCartCount: number;
  retailCartCount: number;
};
...
export function ShopProvider({
  ...
  const [wholesaleWishlist, setWholesaleWishlist] =
    useState<ShopProduct[]>([]);

  const [retailWishlist, setRetailWishlist] =
    useState<ShopProduct[]>([]);

  const [wholesaleCart, setWholesaleCart] =
    useState<CartItem[]>([]);

  const [retailCart, setRetailCart] =
    useState<CartItem[]>([]);
  ...
  const wholesaleCartCount = useMemo(
    () => wholesaleCart.length,
    [wholesaleCart]
  );

  const retailCartCount = useMemo(
    () => retailCart.length,
    [retailCart]
  );
