...
export default function Hero() {
  const { media, loading } = useWebsiteMedia();
  ...
  const mediaRows = media as WebsiteMediaRow[];

  const heroSlides = useMemo<HeroSlide[]>(() => {
    const databaseSlides = mediaRows
      .filter(
        (row) =>
          row.section === "hero" &&
          row.slot_key === "hero-slide" &&
          row.is_active !== false &&
          Boolean(row.desktop_url || row.mobile_url)
      )
      .sort(
        (firstSlide, secondSlide) =>
          Number(firstSlide.display_order ?? 0) -
          Number(secondSlide.display_order ?? 0)
      )
      .map((row) => ({
        id: row.id,
        desktopUrl:
          row.desktop_url ?? row.mobile_url ?? "",
        mobileUrl:
          row.mobile_url ?? row.desktop_url ?? "",
        displayOrder: Number(row.display_order ?? 0),
      }));

    if (databaseSlides.length > 0) {
      return databaseSlides;
    }

    return [
      {
        id: "fallback-slide",
        desktopUrl: heroFallback,
        mobileUrl: heroFallback,
        displayOrder: 0,
      },
    ];
  }, [mediaRows]);

  const heroSettingsRow = useMemo(
    () =>
      mediaRows.find(
        (row) =>
          row.section === "hero" &&
  ...
  const autoplay = getSettingBoolean(
    heroSettingsRow?.settings ?? null,
    "autoplay",
    true
  );

  const slideInterval = Math.max(
    2000,
    getSettingNumber(
      heroSettingsRow?.settings ?? null,
      "interval",
  ...
  const currentSlide =
    heroSlides[currentSlideIndex] ?? heroSlides[0];
    ...
    >
      ...
      <div className="hero-card">
        ...
        <div className="hero-scroll-indicator">
          <span>
            {loading ? "Loading..." : "Scroll to Explore"}
          </span>
